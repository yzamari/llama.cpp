...
extern "C" {
    ...
    struct llama_context_params {
        ...
        bool kv_turboquant; // [EXPERIMENTAL — TurboQuant fork] route KV cache through TurboQuant
                            // (PolarQuant + 1-bit QJL). When true, type_k/type_v are ignored.
                            // Falls back to FP16 in the upstream-merged version of this fork until
                            // the substitution algorithm lands; the field is plumbed end-to-end so
                            // callers can opt in today and benefit transparently when the algorithm
                            // is enabled.
